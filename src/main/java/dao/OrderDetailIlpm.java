...
public class OrderDetailIlpm implements OrderDetailDAO {
	...
	public List<Cart> getAllCart(String orderId) {
		...
		Query query = session.createQuery(
				"SELECT c.id, c.color, s.id, s.ram, s.storage, p.proId, p.proName, p.picture, p.salePrice, p.price, p.discount, od.quantity FROM OrderDetail  od JOIN Product p ON od.productId = p.proId JOIN Color c ON od.colorId = c.id JOIN Storage s ON od.storageId = s.id WHERE od.orderId = :id");
